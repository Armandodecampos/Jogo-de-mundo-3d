// MIT License
// Original file https://github.com/schteppe/cannon.js/blob/908aa1e954b54d05a43d
d708584e882dfe30ae29/tools/threejs/CannonDebugRenderer.js CopyRight https://gith
ub.com/schteppe
  ...
  private _createMesh(shape: CANNON.Shape): THREE.Mesh | THREE.Points {
    ...
    switch (shape.type) {
      ...
      case CANNON.Shape.types.CYLINDER:
        geometry = new THREE.CylinderGeometry((shape as CANNON.Cylinder).radiusT
op, (shape as CANNON.Cylinder).radiusBottom, (shape as CANNON.Cylinder).height,
(shape as CANNON.Cylinder).numSegments)
      ...
      case CANNON.Shape.types.CONVEXPOLYHEDRON:
        ...
        points = []
        for (let i = 0; i < (shape as CANNON.ConvexPolyhedron).vertices.length;
i += 1) {
      ...
      case CANNON.Shape.types.TRIMESH:
        ...
        for (let i = 0; i < (shape as CANNON.Trimesh).vertices.length; i += 3) {
          points.push(new THREE.Vector3((shape as CANNON.Trimesh).vertices[i], (
shape as CANNON.Trimesh).vertices[i + 1], (shape as CANNON.Trimesh).vertices[i +
 2]))
        }
        geometry.setFromPoints(points)
        mesh = new THREE.Mesh(geometry, material)
...
+) {
          for (let yi = 0; yi < (shape as CANNON.Heightfield).data[xi].length -
1; yi++) {
            for (let k = 0; k < 2; k++) {
              ;(shape as CANNON.Heightfield).getConvexTrianglePillar(xi, yi, k =
== 0)
              ...
              points.push(new THREE.Vector3(v0.x, v0.y, v0.z), new THREE.Vector3
(v1.x, v1.y, v1.z), new THREE.Vector3(v2.x, v2.y, v2.z))
  ...
  private _scaleMesh(mesh: THREE.Mesh | THREE.Points, shape: CANNON.Shape) {
    ...
    switch (shape.type) {
      ...
      case CANNON.Shape.types.BOX:
        ...
        mesh.scale.copy(new THREE.Vector3(halfExtents.x, halfExtents.y, halfExte
nts.z))
