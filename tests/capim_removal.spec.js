...
test('Capim removal area verification', async ({ page }) => {
  ...
  // Setup capim and check it exists
  const initialCheck = await page.evaluate(() => {
    const pos = new window.THREE.Vector3(10, 0, 10);
    // Ensure there is some capim at (10, 10)
    ...
    if (window.capimClusters.filter(c => window.calculateWrappedDistance(pos, c.position) < 2.0).length === 0) {
        // Force spawn some capim if none exists
        for(let i=0; i<5; i++) {
  ...
  const result = await page.evaluate(() => {
    ...
    // Verify capim is STILL THERE (new behavior: delayed removal)
